...
describe('main dashboard product sweep contract', () => {
  it('keeps Tech Stack, How To Use, and Companies horizontally grouped on desktop', () => {
    expect(controlCenterPage).toContain('grid gap-4 lg:grid-cols-3')
    expect(controlCenterPage).toContain('Tech Stack')
    expect(controlCenterPage).toContain('How To Use')
    expect(controlCenterPage).toContain('CompaniesPanel')
  })

  it('uses task-ledger work activity with read-only branch metadata and line-count note', () => {
    expect(controlCenterPage).toContain('ActivityMapPanel')
